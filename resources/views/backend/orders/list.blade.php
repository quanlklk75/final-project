...
@section("title-description", "Order List")
@section('content')
    <?php $open = "order"?>
        ...
        <table class="table table-striped" id="myTable">
            ...
            @foreach($orders as $item)
                <tr>
                    ...
                    <td>
                        @if($item->status == 0)
                            <a onclick="return confirm('Make sure you confirm the payment of the order ?')"
                               href="{{ route('order.xac_nhan_thanh_toan', $item->id) }}" class="btn btn-info btn-xs"><i
                                    class="fa fa-edit"></i> Payment confirmation</a>
                        @endif
                        <a href="{{ route('order.get_detail', $item->id) }}" class="btn btn-primary btn-xs"><i
                                class="fa fa-eye"></i> See details</a>
                        <a onclick="return confirm('You definitely delete this product ?')"
